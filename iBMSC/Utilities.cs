...
namespace iBMSC.Editor
{
    ...
    {
        private static System.Globalization.NumberFormatInfo _WriteDecimalWithDot_nfi = new System.Globalization.NumberFormatInfo();
        ...
        {
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.Default))
                return "System Ansi";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.Unicode))
                return "Little Endian UTF16";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.ASCII))
                return "ASCII";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.BigEndianUnicode))
                return "Big Endian UTF16";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.UTF32))
                return "Little Endian UTF32";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.UTF7))
                return "UTF7";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.UTF8))
                return "UTF8";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.GetEncoding(932)))
                return "SJIS";
            if (ReferenceEquals(TextEncoding, System.Text.Encoding.GetEncoding(51949)))
                return "EUC-KR";
            return "ANSI (" + TextEncoding.EncodingName + ")" + ReferenceEquals(TextEncoding, System.Text.Encoding.Default);
        }
        ...
        public static Color AdjustBrightness(Color cStart, float iPercent, float iTransparency)
        {
            ...
            {
                return Color.FromArgb((int)Math.Round(cStart.A * iTransparency), (int)Math.Round((double)(cStart.R * (100f - Math.Abs(iPercent))) * 0.01d + (double)Math.Abs(Conversions.ToInteger(iPercent >= 0f) * iPercent) * 2.55d), (int)Math.Round((double)(cStart.G * (100f - Math.Abs(iPercent))) * 0.01d + (double)Math.Abs(Conversions.ToInteger(iPercent >= 0f) * iPercent) * 2.55d), (int)Math.Round((double)(cStart.B * (100f - Math.Abs(iPercent))) * 0.01d + (double)Math.Abs(Conversions.ToInteger(iPercent >= 0f) * iPercent) * 2.55d));
            }
        }
